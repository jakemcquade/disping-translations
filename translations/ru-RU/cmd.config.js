module.exports = {
  ...
  limit:
    'Вы достигли лимита каналов, которые вы можете добавить с Disping. Этот лимит существует, чтобы каждый пользователь мог наслаждаться одним и тем же опытом Disping. Спасибо за понимание!',
  ...
  saveError:
    'Произошла ошибка при сохранении. Пожалуйста, перенастройте значение, которое вы пытались изменить. Если вы продолжите получать эту ошибку, пожалуйста, свяжитесь с [поддержкой нажав сюда]({support}). Приносим извинения за неудобства.',
  format: {
    spotify:
      'Неправильный URL-адрес. Ссылки Spotify обычно похожи на это: **https://open.spotify.com/artist/ID_GOES_HERE**.',
  ...
  perms: {
    invalid:
      '{alert} • У вас нет разрешения (MANAGE_GUILD) на управление настройками этого сервера для Disping. Обратитесь к владельцу сервера, чтобы запросить это разрешение.',
  },
  buttons: {
